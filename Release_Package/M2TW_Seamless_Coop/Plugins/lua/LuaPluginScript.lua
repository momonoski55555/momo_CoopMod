...
-- Configuration
local pipe_path = "\\\\.\\pipe\\cruesader_pipe"
...
function handle_turn_upload()
    ...
    if response == "UPLOAD_OK" then
        ...
        print("[Lua] Server reported upload issue.")
    end
end

function handle_turn_download(target_turn)
    -- Send format: "DOWNLOAD:5"
    local command = "DOWNLOAD:" .. tostring(target_turn)
    local response = send_pipe_command(command)

    if response and string.sub(response, 1, 5) == "LOAD:" then
        -- Extract path: "LOAD:C:\Path\To\File.sav"
        local path_to_load = string.sub(response, 6)
        print("[Lua] Rust requested load of: " .. path_to_load)
        
        -- PARSE TURN NUMBER from filename to sync state
        -- Expected filename: "turn_123.sav"
        local extracted_turn = string.match(path_to_load, "turn_(%d+)%.sav")
        if extracted_turn then
            turn_number = tonumber(extracted_turn)
            print("[Lua] State Synced: Current Turn set to " .. turn_number)
        end

        -- Load the game
        M2TWEOP.loadGame(path_to_load)
    else
        print("[Lua] Download failed or invalid response.")
    end
end
...
function onFactionTurnEnd() 
    -- Increment turn logic
    turn_number = turn_number + 1
    print("[Lua] Turn End. New Turn: " .. turn_number)
    
    -- Perform the Save -> Rename -> Upload workflow
    handle_turn_upload()
end
...
function draw(pDevice)
    ...
    -- Debug / Control Menu
    ImGui.Begin("Cloud Save Manager", true)
